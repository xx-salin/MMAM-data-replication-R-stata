...
program define clean_family3
    ...
    * Standard fields: destring to float is fine here (no precision loss observed)
    destring duration riskTaking statKnow regretExAnte regretExPost Q14_1_1 Q14_1_2 r1beliefs_alt1_1 r1beliefs_alt1_2 r1beliefs_alt1_3 r1beliefs_alt2_1 r1beliefs_alt2_2 r1beliefs_alt2_3 choice_strength_r1 choiceB_r1, replace force
    ...
    * na.omit() on core columns only — BEFORE view-time coalescing, matching R's order of operations
    egen nmiss = rowmiss(bonus_payment choiceB cond_beliefs_choice_order riskTaking statKnow regretExAnte regretExPost duration beliefA beliefB beliefA_below beliefA_equal beliefA_above beliefB_below beliefB_equal beliefB_above choiceStr whichFirst a3Col short round)
    drop if nmiss > 0
    drop nmiss
