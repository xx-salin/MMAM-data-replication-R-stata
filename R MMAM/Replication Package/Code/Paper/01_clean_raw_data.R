...
## Machine-specific fallback: only used if rprojroot can't locate the repo
## root from the current working directory (e.g. R started in "Documents"
## instead of the repo folder). Harmless on other machines - dir.exists()
## will just be FALSE there and this branch is skipped.
local_root_fallback <- "C:/Users/salins4/OneDrive - Aalto University/Desktop/MMAM/Replication Package"
...
find_repo_root <- function() {
  ...
  tryCatch(
    ...
    error = function(e) {
      if (dir.exists(local_root_fallback)) {
        message("rprojroot couldn't find the repo root from the current ",
                "working directory; using local_root_fallback instead.")
        return(normalizePath(local_root_fallback))
      }
      stop(e)
    }
  )
...
## Clear any pre-existing output before regenerating, so a rerun can't leave
## stale files behind (e.g. from a prior version of this script). Set
## CLEAR_CLEAN_OUT_DIR=FALSE (env var) to skip this if you ever want to
## preserve existing files instead.
if (as.logical(Sys.getenv("CLEAR_CLEAN_OUT_DIR", unset = "TRUE"))) {
  old_files <- list.files(clean_dir, pattern = "\\.csv$", full.names = TRUE, recursive = TRUE)
  if (length(old_files) > 0) {
    message("Clearing ", length(old_files), " existing .csv file(s) from ", clean_dir)
