...
find_repo_root <- function() {
  ...
  tryCatch(
    ...
    error = function(e) {
      if (dir.exists(local_root_fallback)) return(normalizePath(local_root_fallback))
      stop(e)
    }
  )
}
...
out_file <- file.path(clean_dir, "samplingSequences_long.csv")
if (file.exists(out_file)) file.remove(out_file)
write.csv(long, out_file, row.names = FALSE, quote = FALSE)
